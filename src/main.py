...
def main():
    ...
    if sim == "Y":
        ...
        if M is not None:
            ...
            print("Need to specify M when simulating!")
            exit(1)
    ...
    # print UNITY header
    ID = options.id
    outfname="out.%s.%d" % (ID, seed)
    f = open(outdir+'/'+outfname, 'w')
    ...
    chainfname = "chain.%s.%d" % (ID, seed)
    f_chain = open(outdir + '/'+chainfname, 'w')
